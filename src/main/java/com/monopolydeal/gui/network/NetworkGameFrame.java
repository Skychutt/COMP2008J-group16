...
import com.monopolydeal.gui.board.ControlPanel;
import com.monopolydeal.gui.board.GameBoardPane;
import com.monopolydeal.gui.board.GameFrame;
import com.monopolydeal.gui.board.GamePanelHost;
import com.monopolydeal.gui.board.PlayerPanel;
import com.monopolydeal.gui.board.PropertyAreaPanel;
import com.monopolydeal.gui.board.PropertyColorChooser;
import com.monopolydeal.gui.board.RecentLogPanel;
import com.monopolydeal.gui.theme.ThemedDialog;
import com.monopolydeal.gui.board.TopStatusPanel;
...
public class NetworkGameFrame implements GamePanelHost {
    ...
    public static void openAsClient(String host, int port, String roomCode, String playerName,
        ...
        // Show a connection progress dialog
        ConnectionProgressDialog progressDialog = new ConnectionProgressDialog(homeStage);
        progressDialog.setStatus("Connecting to " + host + ":" + port + "...");

        Thread connectThread = new Thread(() -> {
            try {
                Platform.runLater(() -> progressDialog.setStatus("Establishing TCP connection..."));
                ...
                Platform.runLater(() -> {
                    ...
                    progressDialog.setStatus("Connected! Loading game...");
    ...
    /**
     * Opens the network game frame with an already-connected GameClient.
     * Used by the host — the frame is created and wired but NOT shown immediately.
     * It will show itself when it receives the GAME_START or GAME_STATE message.
     */
    public static void openWithClient(GameClient client, Stage homeStage,
                                      boolean hideHomeOnConnect, Runnable homeCallback) {
        NetworkGameFrame frame = new NetworkGameFrame(client, homeStage, homeCallback, hideHomeOnConnect);
        // The client already completed the handshake so onConnected was never dispatched
        // to our listener. Manually sync the player index and update the status.
        frame.myPlayerIndex = client.getMyPlayerIndex();
        frame.topStatusPanel.showWaitingMessage("Connected as " + client.getMyPlayerName()
                + " | Room " + client.getRoomCode() + " — waiting for players...");
    }
